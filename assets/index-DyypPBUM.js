(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`https://docs.google.com/spreadsheets/d/1Hv_suFrUYa5ZInJrYbOLkGCYwJv4A7w_YAyysgMZlAk/export?format=csv&gid=535447968`,t=`dashboard`,n=`05/2026`,r=`all`,i={},a={},o=[],s={},c=``,l=155e5,u=62e5;try{i=JSON.parse(localStorage.getItem(`hospital_salary_data`))||{},a=JSON.parse(localStorage.getItem(`hospital_overtime_data`))||{},o=JSON.parse(localStorage.getItem(`hospital_salary_headers`))||[],s=JSON.parse(localStorage.getItem(`hospital_dependent_overrides`))||{}}catch{i={},a={},o=[],s={}}function d(){localStorage.setItem(`hospital_salary_data`,JSON.stringify(i)),localStorage.setItem(`hospital_overtime_data`,JSON.stringify(a)),localStorage.setItem(`hospital_salary_headers`,JSON.stringify(o)),localStorage.setItem(`hospital_dependent_overrides`,JSON.stringify(s))}var f=e=>{if(!e||!e.name)return!1;let t=String(e.name).trim();return t.length>3&&isNaN(t)&&/[a-zA-ZÀ-ỹ]/.test(t)&&!t.startsWith(`Tổng`)};function p(e){if(typeof e==`number`)return e;if(!e)return 0;let t=e.toString().replace(/[^\d]/g,``);return parseInt(t)||0}function m(e,t=``){try{if(!e)return``;let n=e.trim(),r=n.match(/\/d\/(.*?)(\/|$)/);if(!r)return n;let i=`https://docs.google.com/spreadsheets/d/${r[1]}/export?format=csv`;if(t)i+=`&gid=${t}`;else{let e=n.match(/gid=([\d]+)/);e&&(i+=`&gid=${e[1]}`)}return i}catch{return e}}function h(e){return(e||0).toLocaleString(`vi-VN`)}function g(e){let t=Papa.parse(e,{skipEmptyLines:!0}).data,n=t.findIndex(e=>e.some(e=>e&&(e.toString().trim()===`TT`||e.toString().trim()===`STT`)));n===-1&&(n=6),o=t[n]?t[n].map(e=>e.toString().trim()):[];let r=[];for(let e=n+1;e<t.length;e++){let n=t[e],i=String(n[1]||``).trim();if(!f({name:i}))continue;let a={base:n[4],area:n[5],vkhung:n[6],position:n[7],responsibility:n[8],incentive:n[9],toxic:n[10],party:n[11]},o=[p(n[13]),p(n[14]),p(n[15]),p(n[16]),p(n[17]),p(n[18]),p(n[19]),p(n[20]),p(n[21]),p(n[22]),p(n[23]),p(n[24]),p(n[25]),p(n[26]),p(n[27]),p(n[28]),p(n[29]),p(n[30]),p(n[31])];r.push({id:String(n[0]||``).trim(),name:i,department:String(n[2]||``).trim(),position:String(n[3]||``).trim(),coefficients:a,rawAmounts:o,numDependents:p(n[27])||0,total:o[8]||0,net:o[18]||0})}return r}async function _(){if(i[n]&&i[n].length>0){E();return}E();try{let t=g(await(await fetch(`https://corsproxy.io/?${encodeURIComponent(m(e))}`)).text());t.length&&(i[n]=t,d())}catch(e){console.error(e)}finally{E()}}var v=()=>`
  <aside class="sidebar">
  ...
  </div>`;function w(e){let t=Papa.parse(e,{skipEmptyLines:!0}).data;if(!t.length)return[];let n=t.findIndex(e=>e.some(e=>e&&(e.toString().trim()===`TT`||e.toString().trim()===`STT`)));n===-1&&(n=6);let r=(t[n]?t[n].map(e=>e.toString().trim()):[]).findIndex(e=>e.includes(`Tổng lĩnh`)||e===`Cộng`);r===-1&&(r=20);let i=[];for(let e=n+1;e<t.length;e++){let a=t[e];if(!a[1]||a[1].includes(`Tổng cộng`)||a[1].trim()===``||a[1]===`Họ và tên`)continue;let o=a[1].match(/^(.*?)\s*\((.*?)\)$/),s=o?o[1].trim():a[1].trim(),c=o?o[2].trim():``;i.push({id:a[0]||e-n,name:s,dept:c,salary:p(a[5])||0,hourly:p(a[6])||0,h150d:a[7]||0,m150d:p(a[8])||0,h150n:a[9]||0,m150n:p(a[10])||0,h200d:a[11]||0,m200d:p(a[12])||0,h200n:a[13]||0,m200n:p(a[14])||0,h300d:a[15]||0,m300d:p(a[16])||0,h300n:a[17]||0,m300n:p(a[18])||0,amount:p(a[r])||0})}return i}var T=()=>{let e=(i[n]||[]).filter(f),t=e.reduce((e,t)=>e+t.total,0),r=e.reduce((e,t)=>e+t.net,0);return`<div class="fade-in">${y(`Tổng quan `+n)}<div class="stats-grid"><div class="card stat-card"><span class="stat-label">Tổng quỹ lương</span><span class="stat-value">${h(t)}</span></div><div class="card stat-card"><span class="stat-label">Tổng thực lĩnh</span><span class="stat-value">${h(r)}</span></div><div class="card stat-card"><span class="stat-label">Nhân sự</span><span class="stat-value">${e.length}</span></div></div></div>`},E=()=>{let o=document.getElementById(`app`);if(!o)return;let l=``;switch(t){case`salary`:l=b();break;case`pit`:l=x();break;case`overtime`:l=S();break;case`bonus`:l=C(`Quản lý Khen thưởng`);break;default:l=T()}o.innerHTML=`${v()}<main class="main-content">${l}</main><div id="import-modal" class="modal-overlay" style="display:none;"><div class="card modal-content" style="max-width:500px;"><h2 id="import-title">Import dữ liệu</h2>
    <div style="margin: 1rem 0;">
      <label style="display:block;margin-bottom:0.5rem;font-weight:600;">Tên tháng (MM/YYYY):</label>
      <input type="text" id="import-month-name" class="select-input" style="width:100%;" value="${n}">
    </div>
    <div style="margin-bottom: 1rem;">
      <label style="display:block;margin-bottom:0.5rem;font-weight:600;">Link Google Sheets:</label>
      <input type="text" id="import-url" class="select-input" style="width:100%;" placeholder="Dán link trình duyệt tại đây">
    </div>
    <div style="margin-bottom: 1.5rem;">
      <label style="display:block;margin-bottom:0.5rem;font-weight:600;">ID của Sheet (GID):</label>
      <input type="text" id="import-gid" class="select-input" style="width:100%;" placeholder="Ví dụ: 2041249704 (để trống nếu lấy sheet đầu tiên)">
      <p style="font-size:0.75rem;color:var(--text-muted);margin-top:0.3rem;">Tìm số này ở cuối link trình duyệt (phần gid=...)</p>
    </div>
    <div style="display:flex;gap:1rem;justify-content:flex-end;"><button class="btn btn-secondary" id="close-modal">Hủy</button><button class="btn btn-primary" id="confirm-import">Bắt đầu Import</button></div></div></div>`,lucide.createIcons(),document.querySelectorAll(`.nav-item[data-tab]`).forEach(e=>e.onclick=()=>{t=e.dataset.tab,E()});let u=document.getElementById(`search-input`);u&&(u.value=c,u.oninput=e=>{c=e.target.value,E()});let f=document.getElementById(`month-selector`);f&&(f.onchange=e=>{n=e.target.value,E()});let p=document.getElementById(`pit-quarter-selector`);p&&(p.onchange=e=>{r=e.target.value,E()}),document.querySelectorAll(`.npt-input`).forEach(e=>{e.onchange=e=>{let t=e.target.dataset.name,n=parseInt(e.target.value)||0;s[t]=n,d(),E()}});let h=document.getElementById(`import-btn`),_=document.getElementById(`import-modal`),y=document.getElementById(`close-modal`),D=document.getElementById(`confirm-import`);h&&(h.onclick=()=>_.style.display=`flex`),y&&(y.onclick=()=>_.style.display=`none`),D&&(D.onclick=async()=>{let e=document.getElementById(`import-month-name`).value.trim(),t=document.getElementById(`import-url`).value.trim(),r=document.getElementById(`import-gid`).value.trim(),o=D.getAttribute(`data-type`)||`salary`;if(!e||!t)return alert(`Thiếu thông tin`);D.textContent=`Đang xử lý...`,D.disabled=!0;try{let s=m(t,r),c=await(await fetch(`https://corsproxy.io/?${encodeURIComponent(s)}`)).text();if(o===`salary`){let t=g(c);if(!t.length)throw Error(`Dữ liệu không hợp lệ`);i[e]=t}else{let t=w(c);if(!t.length)throw Error(`Dữ liệu không hợp lệ`);a[e]=t}n=e,d(),_.style.display=`none`,E()}catch(e){alert(`Lỗi: `+e.message)}finally{D.textContent=`Bắt đầu Import`,D.disabled=!1}});let O=document.getElementById(`import-ot-btn`);O&&(O.onclick=()=>{document.getElementById(`import-title`).textContent=`Import Ngoài giờ`,document.getElementById(`import-url`).value=`https://docs.google.com/spreadsheets/d/1d4VhrIM_lk8BeODjG2_PCAK85NXOVI6aLQO1XlUjyiU/edit`,document.getElementById(`import-gid`).value=`2041249704`,D.setAttribute(`data-type`,`overtime`),_.style.display=`flex`}),h&&(h.onclick=()=>{document.getElementById(`import-title`).textContent=`Import Lương`,document.getElementById(`import-url`).value=e,document.getElementById(`import-gid`).value=``,D.setAttribute(`data-type`,`salary`),_.style.display=`flex`});let k=document.getElementById(`ot-month-selector`);k&&(k.onchange=e=>{n=e.target.value,E()});let A=document.getElementById(`theme-toggle`);A&&(A.onclick=()=>{document.body.setAttribute(`data-theme`,document.body.getAttribute(`data-theme`)===`dark`?`light`:`dark`)}),window.lucide&&window.lucide.createIcons()};window.deleteMonth=function(){if(confirm(`Bạn có chắc chắn muốn xóa dữ liệu tháng `+n+`?`)){delete i[n];let e=Object.keys(i);n=e.length?e[0]:`05/2026`,d(),E(),alert(`Đã xóa thành công!`)}},window.exportSalaryToExcel=function(){try{let e=(i[n]||[]).filter(f);if(!e.length)return alert(`Không có dữ liệu để xuất!`);let t=[`Lương chính`,`PC vượt khung`,`PC Khu vực`,`PC Chức vụ`,`PC Trách nhiệm`,`PC ưu đãi ngành`,`PC Độc hại`,`PC cấp ủy`,`Tổng cộng lương`,`Khấu trừ 10,5% BH`,`KT 10,5% BH CV`,`KT 10,5% BH VK`,`Trừ ốm LC`,`Trừ ốm VK`,`Trừ ốm CV`,`Trừ ốm TN`,`Trừ ốm ƯĐ`,`Trừ ốm ĐH`,`Tổng lĩnh`],r=e.map(e=>{let n={TT:e.id,"Họ tên":e.name,"Khoa/Phòng":e.department,"Chức vụ":e.position,HSL:e.coefficients?.base,KV:e.coefficients?.area,VK:e.coefficients?.vkhung,CV:e.coefficients?.position,TN:e.coefficients?.responsibility,ƯĐ:e.coefficients?.incentive,ĐH:e.coefficients?.toxic,CU:e.coefficients?.party};return t.forEach((t,r)=>{n[t]=(e.rawAmounts||[])[r]||0}),n}),a=XLSX.utils.json_to_sheet(r),o=XLSX.utils.book_new();XLSX.utils.book_append_sheet(o,a,`Bang Luong`),XLSX.writeFile(o,`Bang_Luong_${n.replace(`/`,`-`)}.xlsx`)}catch(e){console.error(`Excel Export Error:`,e),alert(`Lỗi khi xuất Excel: `+e.message)}},window.exportSalaryToPDF=function(){try{let e=document.querySelector(`.salary-detail-table`);if(!e)return alert(`Không tìm thấy bảng dữ liệu!`);let t=document.createElement(`div`);t.style.padding=`20px`,t.style.background=`#fff`,t.innerHTML=`
