    ...
    <div class="card">
      ...
      <div class="table-container" style="max-height:650px;">
        <table class="salary-detail-table">
          <thead>
            <tr>
              <th class="sticky-col">TT</th>
              <th class="sticky-col">Họ tên</th>
              <th class="sticky-col">Khoa/Phòng</th>
              <th>Mức 150%</th>
              <th>Mức 200%</th>
              <th>Mức 300%</th>
              <th class="highlight-total">Tổng lĩnh</th>
            </tr>
          </thead>
          <tbody>
            ${t.length?t.map(e=>`<tr>
            ...
            </tr>`).join(``):`<tr><td colspan="7" style="text-align:center;padding:2rem;color:var(--text-muted);">Chưa có dữ liệu ngoài giờ tháng này.</td></tr>`}
          </tbody>
          <tfoot>
            <tr style="font-weight:700;background:var(--card-bg);border-top:2px solid var(--accent);">
              <td></td><td>Tổng cộng</td><td></td>
              <td>${h(t.reduce((e,t)=>e+(t.amount150||0),0))}</td>
              <td>${h(t.reduce((e,t)=>e+(t.amount200||0),0))}</td>
              <td>${h(t.reduce((e,t)=>e+(t.amount300||0),0))}</td>
