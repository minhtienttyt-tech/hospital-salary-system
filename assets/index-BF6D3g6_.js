    ...
    <div class="card">
      ...
      <div class="table-container" style="max-height:650px;">
        <table class="salary-detail-table">
          ...
          <tbody>
            ${e.length>0?e.map((e,t)=>{if(d===`monthly`){let n=t=>{if(e.categoryKey===t)return`selected`;let n=e.limit||e.amount;return t===`TS_CKII`&&n===2e6||t===`THS_CKI_BSNT`&&n===15e5||t===`BS_TYT_DBKK`&&n===12e5||t===`BS_TYT`&&n===1e6?`selected`:``},r=e.limit||e.amount,i=[2e6,15e5,12e5,1e6].includes(r),a=!i&&!e.categoryKey||e.categoryKey===`CUSTOM`,o=e.limit===void 0?i?r:e.amount:e.limit,s=e.months===void 0?1:e.months;return`
                    <tr>
                      <td>${t+1}</td>
                      ...
                      <td>
                        <select class="select-input" onchange="window.updateNQ20Category('${e.name}', this.value)" style="width:100%;font-size:0.85rem;padding:4px 8px;">
                          ...
                          <option value="CUSTOM" ${a?`selected`:``}>Khác (Tùy chỉnh số tiền)</option>
                        </select>
                      </td>
                      <td>
                      ...
                      <td>
                        <input type="number" class="select-input" value="${o}" onchange="window.updateNQ20Limit('${e.name}', this.value)" style="width:110px;text-align:right;font-size:0.85rem;padding:2px 6px;" ${a?``:`disabled`}>
                      </td>
                      <td>
                        <input type="text" class="select-input" value="${s}" onchange="window.updateNQ20Months('${e.name}', this.value)" style="width:70px;text-align:center;font-size:0.85rem;padding:2px 6px;">
                      </td>
