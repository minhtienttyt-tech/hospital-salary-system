        ...
        <table class="salary-detail-table">
          ...
          <tbody>
            ${e.length>0?(()=>{let t=e.reduce((e,t)=>((t.rawAmounts||Array(19).fill(0)).forEach((t,n)=>{e[n]=(e[n]||0)+t}),e),Array(19).fill(0));return`<tr class="total-row" style="background-color: #eef2ff; font-weight: bold;">
                ...
                ${d===`summary`?`<td></td>`:``}
                <td colspan="9"></td>
                ${t.map((e,t)=>`<td class="${t===8?`highlight-total`:t===18?`highlight-col`:``}">${k(e)}</td>`).join(``)}
              </tr>`})():``}
            ${e.map(e=>`<tr>
        ...
        <table class="salary-detail-table">
          <thead>
            <tr>
              <th>STT</th>
              <th>Họ tên</th>
              <th>Nội dung khen thưởng ${d===`monthly`?`<button onclick="window.copyDownBonusContent()" title="Sao chép nội dung khen thưởng từ hàng 1 xuống toàn bộ" style="cursor:pointer;background:none;border:none;margin-left:0.5rem;">⬇️</button>`:``}</th>
              <th>Số tiền</th>
            </tr>
          </thead>
          <tbody>
            ${e.length>0?`
              <tr class="total-row" style="background-color: #eef2ff; font-weight: bold;">
                ...
                <td class="highlight-total" style="color:var(--primary);">${k(e.reduce((e,t)=>e+(t.amount||0),0))}</td>
              </tr>
            `:``}
            ${e.length>0?e.map((e,t)=>`<tr>
                  ...
                  <td>${e.name}</td>
                  <td>
                    ${d===`monthly`?`<input type="text" class="content-input" value="${e.content||``}" onchange="window.updateBonusContent('${n}', ${t}, this.value)" style="width:100%; border:1px solid #ddd; padding:4px; border-radius:4px;">`:e.content||``}
                  </td>
                  <td class="highlight-total">${k(e.amount)}</td>
          ...
          <tbody>
            ${e.length>0?`
              <tr class="total-row" style="background-color: #eef2ff; font-weight: bold;">
                ...
                <td style="color:var(--primary);">${k(e.reduce((e,t)=>e+(t.amount||0),0))}</td>
              </tr>
            `:``}
            ${e.length>0?e.map((e,t)=>`<tr><td>${t+1}</td><td>${e.name}</td><td>${k(e.amount)}</td></tr>`).join(``):`<tr><td colspan="3" style="text-align:center;padding:3rem;color:var(--text-muted);">Chưa có dữ liệu ngoài giờ.</td></tr>`}
          </tbody>
        </table>
      </div>
    </div>
      ...
      <div class="table-container" style="max-height:650px;">
        <table class="salary-detail-table">
          ...
          <tbody>
            ${e.length>0?`
              <tr class="total-row" style="background-color: #eef2ff; font-weight: bold;">
                <td colspan="${d===`monthly`?6:4}" style="text-align:center; text-transform:uppercase;">Tổng cộng</td>
                <td class="highlight-total" style="color:var(--primary);">${k(e.reduce((e,t)=>e+(t.amount||0),0))}</td>
                ${d===`monthly`?`<td></td>`:``}
              </tr>
            `:``}
